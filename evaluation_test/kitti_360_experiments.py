...
def precompute_basalt_sequence_data(
    ...
    basalt_poses = utils.read_basalt_pose(
        os.path.join(basalt_pose_dir, f"{seq_str}.csv")
    )
    ...
    ape_baseline = compute_ape_metrics(gt_poses, basalt_poses[valid_index])
...
def precompute_liodom_sequence_data(
    ...
    ape_baseline = compute_ape_metrics(gt_poses, liodom_poses[valid_index])
...
def main():
    parser = argparse.ArgumentParser(
        description="Run trajectory correction experiments for basalt and liodom"
    )
    parser.add_argument(
        "--basalt_pose_dir",
        type=str,
    ...
    max_segment_sizes = [1000]

    # Sequences: 00-10, skipping some

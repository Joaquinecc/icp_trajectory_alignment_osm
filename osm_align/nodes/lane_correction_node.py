...
class LaneCorrectionNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter(
            ...
            '{"min_segment_size": 150, "knn_neighbors": 20, "icp_error_threshold": 1.5, "max_error_consecutive": 50}',
        )

        self.parameters_correction: dict = json.loads(
            self.get_parameter("parameters_correction")
            .get_parameter_value()
            .string_value
        )
        self.odom_topic_to_correct: str = (
            self.get_parameter("odom_topic_to_correct")
            .get_parameter_value()
            .string_value
        )
        self.map_points_filepath: str = (
            self.get_parameter("map_points_filepath").get_parameter_value().string_value
        )
        self.save_resuts_path: str = (
            self.get_parameter("save_resuts_path").get_parameter_value().string_value
        )
        self.initial_gps_topic: str = (
            self.get_parameter("initial_gps_topic").get_parameter_value().string_value
        )
        ...
        self.trajectory_correction: Optional[OdomCorrector] = (
            None  # Trajectory correction
        )

        # Initialize UTM projector and lanelet map
        self.sub_gps = self.create_subscription(
            NavSatFix, self.initial_gps_topic, self.initial_gps_callback, 10
        )
        # Main processing loop
        self.sub_odom = self.create_subscription(
            Odometry, self.odom_topic_to_correct, self.odom_callback, 10
        )

        # Publish corrected odometry
        self.pub_corrected_odom = self.create_publisher(
            Odometry, ODOM_ALIGNED_TOPIC, 10
        )
        ...
        try:
            self.tf_to_map = utils.tf_matrix_from(
                self.tf_buffer, target="enu", source="odom", timeout_sec=5.0
            )
        ...
        self.get_logger().info(
            f"Yaw angle (Z axis, degrees) from odom to enu: {yaw:.2f}"
        )
    ...
    def initial_gps_callback(self, msg: NavSatFix) -> None:
        ...
        if self.map_points_filepath:
            ...
            raise ValueError(
                "No map points filepath provided, trajectory correction not possible"
            )
    ...
    def publish_odom(self, pose, base_frame_id: str) -> None:
        ...
        self.get_logger().debug(
            f"frame {self.frame_count} pose: {pose.position.x}, {pose.position.y}, {pose.position.z}"
        )
    ...
    def _initialize_odom_correction(self) -> None:
        ...
        args = {
            k: self.parameters_correction[k]
            for k in valid_keys
            if k in self.parameters_correction
        }
        self.trajectory_correction = OdomCorrector(self.points_lane_map, args)
        self.get_logger().info("OdomCorrector initialized")

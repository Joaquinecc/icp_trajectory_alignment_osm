...
class OdomEnuCorrectionNode(Node):
    ...
    def __init__(self):
        ...
        self.poses_history: list = (
            []
        )  # List of 4x4 pose matrices (in odom frame) - max n
        self.gps_positions: list = (
            []
        )  # List of GPS positions in ENU (relative to origin) - max n
        ...
        self.pub_odom_enu = self.create_publisher(Odometry, self.odom_enu_topic, 10)
        self.get_logger().info(
            f"Publishing corrected odometry to: {self.odom_enu_topic}"
        )
    ...
    def odom_callback(self, msg: Odometry) -> None:
        ...
        if (
            len(self.poses_history) >= self.n_points
            and len(self.gps_positions) >= self.n_points
            and self.yaw_offset_deg is None
        ):
    ...
    def gps_callback(self, msg: NavSatFix) -> None:
        ...
        if self._utm_projector is None:
            ...
            gps_pos = np.array(
                [float(ref_point.x), float(ref_point.y), float(ref_point.z)]
            )
    ...
    def _try_calculate_yaw_offset(self) -> None:
        ...
        if (
            len(self.poses_history) < self.n_points
            or len(self.gps_positions) < self.n_points
        ):
            return

        # Extract 2D positions (x, y) from odom poses - ignore z/altitude
        odom_points_2d = np.array(
            [pose[:2, 3] for pose in self.poses_history[: self.n_points]]
        )

        # Extract 2D positions (x, y) from GPS - ignore z/altitude
        gps_points_2d = np.array(
            [gps_pos[:2] for gps_pos in self.gps_positions[: self.n_points]]
        )
        ...
        if rotation_matrix is None:
            self.get_logger().warn(
                "Failed to calculate rotation using Kabsch algorithm"
            )
            return
